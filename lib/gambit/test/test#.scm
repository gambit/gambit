  ...
  (lambda (src)
    ...
    (define (expand-check-= src)
      (##deconstruct-call
       src
       -3
       (lambda (expr1 expr2 #!optional (tolerance 'gambit/test#epsilon) comment)
         (let ((msg (failed-check-message src)))
           (##sourcify
            (list (##sourcify 'gambit/test#check-=-proc src)
                  (##sourcify expr1 src)
                  (##sourcify expr2 src)
    ...
    (define (expand-check-exn src tail-exn?)
      (##deconstruct-call
       ...
       (lambda (exn? thunk #!optional comment)
         (let ((msg (failed-check-message src)))
           (##sourcify
            (list (##sourcify 'gambit/test#check-exn-proc src)
                  (##sourcify exn? src)
                  (##sourcify thunk src)
                  (##sourcify msg src)
    ...
    (define (failed-check-generate src actual-result)
      (##sourcify
       (list (##sourcify 'gambit/test#failed-check src)
