...
(define-prim (##vector-fold kons knil vect . vects)
  ...
  (define (smallest-length vector-list len)
    (if (null? vector-list)
        len
...
(define-procedure (vector-fold
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        len
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-fold kons knil vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
                                   len)
                               (##fx+ arg-id 1)))))))
       ...
       (let vector-fold1-loop ((knil knil)
         ...
         (if (fx= i len)
             ...
             (vector-fold1-loop (kons knil (vector-ref vect i))
                                (fx+ i 1)))))
      (else
       (let ((len (smallest-length vects len 4))
             (vects (cons vect vects)))
         (let vector-fold2+-loop ((knil knil)
                                  (i 0))
           (if (fx= i len)
...
(define-procedure (vector-map!
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-map! f vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
       ...
       (let vector-map1!-loop ((i len))
         (if (fx= i 0)
            ...
            (let ((i (fx- i 1)))
              ...
              (vector-map1!-loop i)))))
      (else
       (let* ((len (smallest-length vects len 3))
              (vects (cons vect vects)))
         (let vector-map2+!-loop ((i len))
           (if (fx= i 0)
               vect
         ...
         (if (fx= i len)
             ...
             (let ((count (if (pred? (vector-ref vect i))
               ...
               (vector-count1-loop count
                                   (fx+ i 1))))))
      (else       
       (let* ((len (smallest-vector-length vects (vector-length vect)))
              (vects (cons vect vects)))
...
(define-procedure (vector-count 
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-count pred? vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
         ...
         (if (fx= i len)
             ...
             (let ((count (if (pred? (vector-ref vect i))
               ...
               (vector-count1-loop count
                                   (fx+ i 1))))))
      (else       
       (let* ((len (smallest-length vects (vector-length vect) 3))
              (vects (cons vect vects)))
         (let vector-count2+-loop ((count 0)
                                   (i     0))
           (if (fx= i len)
...
(define-procedure (vector-index
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-index pred? vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
...
(define-procedure (vector-skip
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-skip pred? vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
...
(define-procedure (vector-index-right
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-index-right pred? vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
...
(define-procedure (vector-skip-right
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-skip-right pred? vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
...
(define-procedure (vector-any 
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-any pred? vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
...
(define-procedure (vector-every 
  ...
  (define (smallest-length vect-lst len arg-id)
    (if (null? vect-lst)
        ...
        (let ((vect (car vect-lst)))
          (macro-check-vector
            vect arg-id (vector-every pred? vect . vects)
            (let ((vlen (vector-length vect)))
              (smallest-length (cdr vect-lst) 
                               (if (fx< vlen len)
                                   vlen
