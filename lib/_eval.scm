...
(define (##extract-module-data code comp-ctx)
  (##vector
   code
...
(define (##setup-requirements-and-run data rte)
  ;; data is generated by the ##extract-module-data function
  (let ((code (##vector-ref data 0))
        (demand (##vector-ref data 1))
        (supply (##vector-ref data 2))
        (meta-info (##vector-ref data 3)))

   (if (zero? (length supply))
       ;; We immediately evaluate the code
       (begin
         (##load-modules demand)
         (macro-code-run code))
       ;; We add a registered-module
       (begin
         (let ((the-mod-descr
                (macro-make-module-descr
                 (list->vector supply)
                 (list->vector demand)
                 (table->list meta-info)
                 0
                 (lambda ()
                   (macro-code-run code))
                 #f)))
