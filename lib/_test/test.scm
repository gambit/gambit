...
(check-equal? (failure-msg (lambda () (check-equal? "ab_" "abc")))
              "\"test.scm\"@28.39: FAILED (check-equal? \"ab_\" \"abc\") GOT \"ab_\"\n")
...
(check-equal? (failure-msg (lambda () (check-exn pair?
                                                 (lambda () (/ 1 0)))))
              "\"test.scm\"@84.39: FAILED (check-exn pair? (lambda () (/ 1 0))) GOT #<divide-by-zero-exception #2>\n")
(check-equal? (failure-msg (lambda () (check-exn divide-by-zero-exception?
                                                 (lambda () (/ 1 2)))))
              "\"test.scm\"@87.39: FAILED (check-exn divide-by-zero-exception? (lambda () (/ 1 2))) GOT 1/2\n")
...
(check-equal? (failure-msg (lambda () (check-tail-exn pair?
                                                      (lambda () (/ 1 0)))))
              "\"test.scm\"@96.39: FAILED (check-tail-exn pair? (lambda () (/ 1 0))) GOT #<divide-by-zero-exception #3>\n")
(check-equal? (failure-msg (lambda () (check-tail-exn divide-by-zero-exception?
                                                      (lambda () (/ 1 2)))))
              "\"test.scm\"@99.39: FAILED (check-tail-exn divide-by-zero-exception? (lambda () (/ 1 2))) GOT 1/2\n")
