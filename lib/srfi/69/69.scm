...
(define (alist->hash-table alist #!optional 
                                 (equal-fn (macro-absent-obj)) 
                                 (hash-fn (macro-absent-obj)))
  (if (eq? equal-fn (macro-absent-obj))
      (list->table alist)
...
(define (hash-table-hash-function ht)
    (or (macro-table-hash ht) eq?-hash))
    ...
    (let ((val (table-ref ht key (macro-deleted-obj))))
        (if (eq? val (macro-deleted-obj))
                (if (eq? thunk (macro-absent-obj))
                    (##raise-unbound-key-exception hash-table-ref key)
                    (thunk))
                val)))
...
(define (hash-table-exists? ht key) 
    (not (eq? (hash-table-ref ht key (lambda () (macro-deleted-obj)))
              (macro-deleted-obj))))

(define (hash-table-update! ht key func #!optional (thunk (macro-absent-obj)))
    (let ((val (if (eq? thunk (macro-absent-obj))
                   (hash-table-ref/default ht key (macro-deleted-obj))
                   (hash-table-ref ht key thunk))))
          (if (eq? val (macro-deleted-obj))
              (##raise-unbound-key-exception hash-table-update! key)
              (hash-table-set! ht key (func val)))))

(define (hash-table-update!/default ht key func default)
    (let ((val (hash-table-ref/default ht key default)))
         (hash-table-set! ht key (func val))))
