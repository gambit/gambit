...
(check-equal? (hash-table-hash-function (make-hash-table)) ##equal?-hash)
...
(check-equal? (hash-table-ref (alist->hash-table '((0 . 0) (1 . 1))) 0) 0)

(check-equal? (hash-table-ref (alist->hash-table '((0 . 0) (1 . 1))) 2 (lambda() #f)) #f)

(check-tail-exn unbound-key-exception? 
    (lambda () (hash-table-ref (alist->hash-table '((0 . 0) (1 . 1))) 2 
        (lambda () (##raise-unbound-key-exception hash-table-ref 'test 'test)))))
...
(check-equal? (hash-table-ref/default (alist->hash-table '((0 . 0) (1 . 1))) 0 #f) 0)

(check-equal? (hash-table-ref/default (alist->hash-table '((0 . 0) (1 . 1))) 2 #f) #f)
...
(check-tail-exn unbound-key-exception?
    (lambda () (hash-table-update! 
        (make-hash-table) 0 (lambda (val) (+ val 1)))))
...
(let ((ht (alist->hash-table '((0 . 2) (1 . 3)))))
    (check-true (fold (lambda (elem res) 
        (if (equal? res #t) #t 
            (table-search (lambda (key val) (if (equal? val elem) #t #f)) ht))) 
            #f 
