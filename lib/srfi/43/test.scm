...
;;============================================================================
;; vector-fold
(test-assert (= (vector-fold (lambda (i state x)
                               (+ (* state 2) x))
                             0
                             #(1 2 3))
                11))

(test-assert (= (vector-fold (lambda (i state v p)
                                  (+ state (* v p)))
                                0
                                #(1 2 3)
                                #(.25 .5 .25))
...
(let ((vec #(1 2 3)))
  (test-assert (equal? (vector-map (lambda (i x) (* x x))
                                   vec)
                       #(1 4 9)))
  
  (test-assert (equal? vec #(1 2 3)))
  (test-assert (equal? (vector-map (lambda (i x y) (* x y))
                                   vec
                                   vec)
                       #(1 4 9)))
  (test-assert (equal? vec #(1 2 3))))
...
(test-assert (= (vector-count (lambda (i x) (even? x)) 
                              #())
                0))
...
(test-assert (= (vector-count (lambda (i x y)
                                (= (* 2 x) y))
                              #(0 1 2 3)
                              #(0 1 2 3))
                1))
