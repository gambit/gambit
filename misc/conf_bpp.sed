...
s%@srcdirpfx@%%g
s%@o@%obj%g
s%@CC_O@%bcc32 -x -P -w- -O2 -a4 -tWM $(CFLAGS) -c $(CC_O_ARGS) -D___SINGLE_HOST%g
s%@CC_O_SH@%bcc32 -x -P -w- -O2 -a4 -tWM $(CFLAGS) -c $(CC_O_SH_ARGS) -D___SINGLE_HOST%g
s%@LINK@%bcc32 -x -e\\$$EXECUTABLE $(LFLAGS) \\$$OBJECTS ../lib/$(GAMBCLIB) $(CLIBS)%g
s%@LINK_SH@%bcc32 -x -e\\$$EXECUTABLE $(LFLAGS) \\$$OBJECTS ../lib/$(GAMBCLIB_SH)%g
s%@LINK_SH_INSTALL@%bcc32 -x -e\\$$EXECUTABLE $(LFLAGS) \\$$OBJECTS ../lib/$(GAMBCLIB_SH)%g
s%@CLIBS@%%g
