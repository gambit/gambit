...
(define (read-n-chars n port)
  (let loop ((n n) (lst '()))
    (if (> n 0)
        (let ((c (read-char port))) (loop (- n 1) (cons c lst)))
        (reverse lst))))
...
(define (gen-char i mod)
  (integer->char
   (min (##max-char-code)
        (vector-ref
         '#(0 127 128 2047 2048 65520 65536 1114111)
         (modulo i mod)))))
...
(define (test1 port mod)
  (let loop ((i 0))
    (if (< i 1000)
        (begin
          (test-eqv i (output-port-char-position port))
          (if (= 9 (modulo i 10))
              (newline port)
    ...
    (if (< i 1000)
        (begin
          ...
          (if (= 9 (modulo i 10))
              ...
              (display (gen-char i mod) port))
          (if (= 20 (modulo i 21))
              (begin (force-output port) (read-n-chars len port)))
          (loop (+ i 1)))
        (begin
          (close-port port)
          (let ((str (read-line port #f #t)))
            (test-eqv 530 (string-length str)))))))
...
(define (test-str)
  ...
  (test1 (open-string) 8)
  (test2 (open-string) 8)
  (test3 (open-output-string) (lambda (port) (get-output-string port)) 8)
  (test3 (open-string) (lambda (port) (get-output-string port)) 8))

(define (test-u8vect x)
  (let ((char-enc (car x)) (mod (cadr x)))
    ...
    (test3 (open-output-u8vector (list char-encoding: char-enc))
           (lambda (port) (get-output-u8vector port))
           mod)
