...
(define (obj->str->obj obj wr rd) (str->obj (obj->str obj wr) rd))
...
(let ((shared (let* ((a '(123 #\space)) (b (list a a)) (c (vector b b)))
                (cons c '()))))
  (test-equal
   shared
...
(let ((point (make-point 11 22)))
  (test-equal
   point
   (obj->str->obj point (lambda (obj) (write obj)) (lambda () (read)))))

(let ((point3d (make-point3d 11 22 33)))
  (test-equal
   point3d
...
(let* ((eq-table (list->table '((abc . 123) (456 . 789)) test: eq?))
  ...
  (test-equal (table-ref eq-table 'abc) (table-ref deserialized-eq-table 'abc))
  (test-equal (table-ref eq-table 456) (table-ref deserialized-eq-table 456))
  (test-equal
...
(let* ((cont (thread-join!
  ...
  (test-equal ret deserialized-ret)
  
  (test-equal (##frame-fs frame) (##frame-fs deserialized-frame))
  
  (test-equal (##frame-ret frame) (##frame-ret deserialized-frame))
    ...
    (test-eq unique
             (thread-join!
              (thread-start!
               (make-thread
                (lambda ()
                  (continuation-return deserialized-cont unique))))))))

(let ((promise (delay (* 6 7))))
  (test-equal
   (force promise)
   (force (obj->str->obj
           promise
