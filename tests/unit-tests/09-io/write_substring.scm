...
(test-error-tail
 ...
 (write-substring "abcd" 1))
(test-error-tail
 wrong-number-of-arguments-exception?
 (write-substring "abcd" 1 3 (current-output-port) #f))
...
(test-error-tail
 range-exception?
 (write-substring "abcd" -1 3 (current-output-port)))
(test-error-tail
 range-exception?
 (write-substring "abcd" 0 5 (current-output-port)))
(test-error-tail
 range-exception?
 (write-substring "abcd" 3 2 (current-output-port)))
