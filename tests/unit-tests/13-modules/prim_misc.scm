...
(check-same-behavior
 ("" "##" "~~lib/gambit/prim/misc#.scm")
 
 ;; R4RS
 
 (apply ##list (##list 1 2 3))
 (apply ##list 1 2 3 (##list 4 5 6))
 
 (call-with-current-continuation (lambda (k) (k 123)))
 (call-with-current-continuation (lambda (k a) (k (##list a))) 1)
 (call-with-current-continuation (lambda (k a b) (k (##list a b))) 1 2)
 ...
 (##call-with-values (lambda () (values 1)) ##list)
 (##call-with-values (lambda () (values 1 2)) ##list)
 ...
 (apropos)
 (apropos "$_!*%")
 (call-with-output-string (lambda (port) (apropos "$_!*%" port)))
 ...
 (continuation-capture
  (lambda (k) (##cons 42 (continuation-graft k (lambda () 123)))))
 (continuation-capture
  (lambda (k a) (##cons 42 (continuation-graft k ##list a)))
  1)
 (continuation-capture
  (lambda (k a b) (##cons 42 (continuation-graft k ##list a b)))
  1
  2)
 (continuation-capture
  (lambda (k a b c) (##cons 42 (continuation-graft k ##list a b c)))
  1
  2
  3)
 (continuation-capture
  (lambda (k a b c d) (##cons 42 (continuation-graft k ##list a b c d)))
  1
  2
  3
  4)
 ...
 (##call-with-values
  (lambda ()
    (continuation-capture
     (lambda (k a) (##cons 42 (continuation-return k a)))
     1))
  ##list)
 (##call-with-values
  (lambda ()
    (continuation-capture
     (lambda (k a b) (##cons 42 (continuation-return k a b)))
     1
     2))
  ##list)
 (##call-with-values
  (lambda ()
    (continuation-capture
     (lambda (k a b c) (##cons 42 (continuation-return k a b c)))
     1
     2
     3))
  ##list)
 (##call-with-values
  (lambda ()
    (continuation-capture
     (lambda (k a b c d) (##cons 42 (continuation-return k a b c d)))
     1
     2
     3
     4))
 ...
 ;; minimal test
 (and #f
      ;; the race condition might not work on some platforms
      (let* ((c #f)
             (t (thread-start!
                 (make-thread (lambda () (dead-end) (set! c #t))))))
        (thread-sleep! .01)
        (thread-terminate! t)
        c))
