(define-macro (init-compiled-feature!)
  (if (##not (##unbound?
              (##global-var-ref (##make-global-var '##compilation-options))))
...
(cond-expand
 ...
 (else (define loop-delay 37) (define time-scale 100)))
...
(define (start-short-thread)
  (thread-start! (make-thread (lambda () (short-delay)))))

(define (delayed-thread-interrupt! thread thunk delay)
  (let loop ((i delay))
    (if (> i 0) (loop (- i 1)) (thread-interrupt! thread thunk))))
...
(define (go n)
  ...
  (define cycle (quotient (* loop-delay 5) 100))
  ;; 5% variation in delay
            ...
            (with-exception-catcher
             (lambda (exc)
               (cond ((inactive-thread-exception? exc)
                     ...
                     (else (raise exc))))
             (lambda ()
               (delayed-thread-interrupt!
                t
                list
                (+ loop-delay (modulo i cycle))))))
          (loop (+ i 1))))))
