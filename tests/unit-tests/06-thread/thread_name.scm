...
(test-equal (void) (thread-name t1))
(test-equal 't2 (thread-name t2))
(test-equal 't3 (thread-name t3))
(test-equal (void) (thread-name t4))
(test-equal 't5 (thread-name t5))
(test-equal 't6 (thread-name t6))
(test-equal 't7 (thread-name t7))
(test-equal 't8 (thread-name t8))

(test-error-tail uninitialized-thread-exception? (thread-name t10))
(test-error-tail uninitialized-thread-exception? (thread-name t11))
(test-error-tail uninitialized-thread-exception? (thread-name t12))
(test-error-tail uninitialized-thread-exception? (thread-name t13))
...
(test-equal (void) (thread-name t10))
(test-equal 't11 (thread-name t11))
(test-equal 't12 (thread-name t12))
...
(test-error-tail type-exception? (thread-name #f))

(test-error-tail wrong-number-of-arguments-exception? (thread-name))
(test-error-tail wrong-number-of-arguments-exception? (thread-name t1 #f))
