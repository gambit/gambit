...
(define (flood target)
  ...
  (define (body)
    (let loop ((n iterations))
      (if (> n 0) (begin (thread-interrupt! target inc!) (loop (- n 1))))))
  
  (set! threads
        (map (lambda (id) (make-thread body)) (iota nb-competing-threads)))
...
(define (test1)
  (let* ((loop? #t)
         (t (make-thread
             (lambda ()
               (let loop ()
                 ...
                 (thread-yield!)
                 ;; give interrupting threads a chance to run
                 (if loop? (loop)))))))
    ...
    (test-equal
     ...
     (vector-ref (thread-specific t) 0))))

(define (test2)
  (let ((t (make-thread (lambda () (thread-sleep! 1000)))))
...
(define (test3)
  (let* ((m (make-mutex)) (t (make-thread (lambda () (mutex-lock! m)))))
...
(define (test4)
  (let* ((m (make-mutex)) (t (make-thread (lambda () (mutex-lock! m 1000)))))
