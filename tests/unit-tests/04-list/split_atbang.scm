...
(call-with-values
 (lambda () (split-at! (cons 1 (cons 2 3)) 2))
 (lambda (front back) (test-equal '(1 2) front) (test-equal 3 back)))

(test-error-tail range-exception? (split-at! (append lst1 '()) 4))
(test-error-tail range-exception? (split-at! (append lst1 '()) -1))

(test-error-tail wrong-number-of-arguments-exception? (split-at!))
(test-error-tail
 wrong-number-of-arguments-exception?
 (split-at! (append lst1 '())))
(test-error-tail
 wrong-number-of-arguments-exception?
 (split-at! (append lst1 '()) 0 0))
