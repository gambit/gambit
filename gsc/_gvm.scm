...
(define (bb-interpret bbs bb env stack registers)
  ...
  (define (assert-types instr)
    (define bits-to-checker
      (list
        (cons type-false-bit     #f)
        (cons type-true-bit      #f)
        (cons type-null-bit      #f)
        (cons type-void-bit      #f)
        (cons type-eof-bit       #f)
        (cons type-absent-bit    #f)
        (cons type-bignum-bit    #f)
        (cons type-ratnum-bit    #f)
        (cons type-flonum-bit    #f)
        (cons type-cpxnum-bit    #f)
        (cons type-char-bit      #f)
        (cons type-symbol-bit    #f)
        (cons type-keyword-bit   #f)
        (cons type-string-bit    #f)
        (cons type-vector-bit    #f)
        (cons type-u8vector-bit  #f)
        (cons type-s8vector-bit  #f)
        (cons type-u16vector-bit #f)
        (cons type-s16vector-bit #f)
        (cons type-u32vector-bit #f)
        (cons type-s32vector-bit #f)
        (cons type-u64vector-bit #f)
        (cons type-s64vector-bit #f)
        (cons type-f32vector-bit #f)
        (cons type-f64vector-bit #f)
        (cons type-pair-bit      #f)
        (cons type-procedure-bit #f)
        (cons type-box-bit       #f)
        (cons type-promise-bit   #f)
        (cons type-other-bit     #f)))

    (define (compare value expected-type)
      (define (compare-non-fixnum)
        #t)

      (define (compare-fixnum)
        #t)

      (if (not (and (compare-non-fixnum) (compare-fixnum)))
    ...
    (let* ((types (gvm-instr-types instr))
      ...
      (for-each
        (lambda (reg index)
          (compare (register-ref registers reg)
      ...
      (for-each
        (lambda (slot index)
          (compare (stack-ref stack (bb-entry-frame-size bb) slot)
                   (vector-ref types index)))
        (iota n-slots 1)
