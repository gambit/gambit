...
(define (univ-dump-code targ procs output c-intf module-descr unique-name options)
  ...
  (define (defs->code ctx mod-name defs)
    ...
    (if (eq? (target-name (ctx-target ctx)) 'java)
        (let* ((max-field-byte-size 30) ;; Random guess.
               (max-init-byte-size 300) ;; Calculated from the largest inits
                                        ;; generated by univ-lib.
               (max-method-size 65000) ;; JVM limitation.
               (fields-size (* (length (univ-defs-fields defs))
                               max-field-byte-size))
               (inits-size (* (length (univ-defs-inits defs))
                              max-init-byte-size)))
          ;; TODO refine choosen sizes.
          ;; 200 roughly allows 325 bytes per inits.
          ;; If we suppose each inits adds 10 constants to the constant pool,
          ;; we can create at most 30 methods per class.
          (cond
           ((and (> max-method-size fields-size)
                 (< max-method-size inits-size))
            (set! defs (univ-defs-use-initialization-classes ctx defs 200 30)))
           ((> (+ fields-size inits-size) max-method-size)
            (set! defs (univ-defs-separate-field-initialization ctx defs))
            (set! defs (univ-defs-use-initialization-classes ctx defs 200 30))))))
                  ...
                  (if (proc-obj? obj)
                      ...
                      (let ((state (table-ref table obj)))
                        (if (or (> (vector-ref state 0) 1) ;; use a variable?
                                (memq (target-name (ctx-target ctx))
                                      '(java python))) ;; Python and Java can't handle deep nestings
                            (let ((cst
                                   (vector-ref state 2))
                                  (val
...
(define (univ-defs-keep defs field? method? classes? inits?)
  (let ((fields (and field? (univ-defs-fields defs)))
   ...
   (univ-defs
    ...
    (or inits '())
    (apply append
           (keep (lambda (x) x) (list fields methods classes inits))))))
...
(define (univ-defs-use-initialization-classes ctx defs method-size class-size)
  ...
  (define (group->method-body group)
    (let loop ((acc (lambda (ctx) (^)))
               (lst group))
      (if (null? lst)
          acc
          (loop (lambda (ctx) (^ (acc ctx) ((car lst) ctx)))
                (cdr lst)))))

  (define (make-class i methods)
    (univ-class
     (string-append "Init" (number->string i))
     '()
     #f
     '()
     '()
  ...
  (define (make-init-call class)
    (map (lambda (method)
          (lambda (ctx)
           (^expr-statement
             (^call-prim (^member (univ-class-root-name class)
                                  (univ-method-name method))))))
         (univ-class-class-methods class)))

  (let* ((inits (reverse (univ-defs-inits defs)))
         (method-bodies (map group->method-body
                             (group method-size inits)))
         (methods (map (lambda (mbg) (mapi make-method mbg))
                       (group class-size method-bodies)))
         (classes (reverse (mapi make-class methods)))
         (inits (apply append (map (lambda (x) (reverse (make-init-call x)))
                                   classes))))
    (univ-defs-combine
     (univ-defs-combine (univ-defs-keep defs #t #t #t #f)
                        (univ-defs '() '() classes '() classes))
     (univ-defs '() '() '() inits inits))))

(define (univ-defs-separate-field-initialization ctx defs)
  ...
  (define (field->initialization field)
    (lambda (ctx) (^assign (univ-field-name field) (univ-field-init field))))

  (let* ((fields (map field->declaration (univ-defs-fields defs)))
         (field-initializations (map field->initialization
                                     (keep univ-field-init
                                           (univ-defs-fields defs))))
         (inits (append (univ-defs-inits defs) field-initializations))
